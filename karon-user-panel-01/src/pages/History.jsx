...
const StatusBadge = ({ status }) => {
  ...
  return (
    <div
      className={`badge ${styles[status] || "badge-ghost"} font-bold flex items-center p-3`}
    >
      {icons[status]}
      {status}
    </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-4">
        <div>
          ...
          <p className="text-base-content/60">
            View your purchase and return history
          </p>
        </div>
                  ...
                  {order.items.map((item) => (
                    <div key={item.id} className="flex items-center gap-6">
                      ...
                      <div className="hidden md:flex gap-3">
                        <Link
                          to={`/product/${item.id}`}
                          className="btn btn-sm btn-ghost rounded-full px-6"
                        >
                          View Product
                        </Link>
                        {order.status === "Delivered" && (
                          <button className="btn btn-sm btn-outline btn-ghost rounded-full px-6">
                            Return Item
                          </button>
                        )}
                      </div>
